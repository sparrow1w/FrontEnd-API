...
Handlebars.registerHelper("calculateAge", function (dateOfBirthUTC) {
    var birthday = +new Date(dateOfBirthUTC);
...
$(document).ready(function() {
    $('#mytable').on('click-cell.bs.table', function (field, value, row, $element) {
        var zaidejas = $element.PersonId;
        var komanda = $element.TeamId;
        var vardas = $element.Name;
        $(function () {
            var $players = $('#players-stats');
            var url = 'http://cors-anywhere.herokuapp.com/http://data.nba.net/10s/prod/v1/2017/players/' + zaidejas + '_profile.json';
            $.ajax({
                type: 'GET',
                url: url,
        ...
        $(function () {
            ...
            $.ajax({
                ...
                url: url1,
                error: function (team) {
                    $teams.html('<div><img src=src/no-person.png height="190" width="260" ><p>' + vardas + '</p></div>')
                },
                success: function (team) {
                    $teams.html('<div><img src=' + url1 + '><p>' + vardas + '</p></div>')
                }
